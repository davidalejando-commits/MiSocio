...
export function crearCliente(cliente: Omit<Cliente, 'id' | 'created_at'>): number {
  const result = db.runSync(
    `INSERT INTO clientes (nombre, telefono, direccion, foto_uri, limite_credito)
     VALUES (?, ?, ?, ?, ?)`,
    [cliente.nombre, cliente.telefono, cliente.direccion, cliente.foto_uri, cliente.limite_credito]
  );
...
export function registrarAbono(clienteId: number, monto: number, metodo: string) {
  try {
    db.runSync(
      'INSERT INTO abonos (cliente_id, monto, metodo, created_at) VALUES (?, ?, ?, ?)',
      clienteId, monto, metodo, ahoraCol()  // ← timestamp Colombia
    );
    db.runSync(
